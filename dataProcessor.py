...
class dataProcessor(QObject):
    def __init__(self):  # 构造函数
        ...
        self.RotationMatrix = np.array([[1/(3**0.5), -1/(3**0.5), -1/(3**0.5)], [1/(6**0.5), 2**0.5/3**0.5, -1/(6**0.5)], [1/(2**0.5), 0, 1/(2**0.5)]])
    ...
    def set_zero(self, distances: list) -> bool:
        ...
        self.zero_vector = np.transpose(ZeroVector)
        return True
    ...
    def set_axis(self, distancesList: list) -> bool:
        ...
        # 最小二乘法求旋转轴
        RotationAxis = np.dot(np.dot(np.linalg.inv(np.dot(NewMatrixAt, NewMatrixA)), NewMatrixAt), NewMatrix)
        # 旋转轴乘以旋转矩阵，并归一化
        sum = np.linalg.norm(RotationAxis)
        self.axis = RotationAxis / sum
        return True
    ...
    def get_angle(self, distances: list) -> float:
        ...
        newzero_vector = np.dot(self.RotationMatrix, self.zero_vector)
        ...
        cos_angle = NormalVector1.dot(NormalVector2)/(NormalVector1Length*NormalVector2Length)
...
if __name__ == "__main__":
    ...
    print(d)
    e = processor1.set_axis([[0, 0, 0], [10, 0, 0.0002], [-21, 0.0001, 0], [-32, 0.0001, 0.0001], [43, 0.0002, 0.0001]])
    print(e)
